...
class Diffy:
    ...
    def _resolve(self, changeset):  # noqa: C901
        ...
        b, f, s = [changeset.get(src, None) for src in self.sources]
    ...
    def _resolve_conflict_interactive(self, conflict):
        ...
        if conflict.get("base") is None:  # case 5
            ...
            choices = (
                ...
                (
                    "m",
                    "Merge f/s",
                    lambda: self._edit_conflict(conflict["first"], conflict["second"]),
                ),
        ...
        resolved = self._interact("How should I handle the conflict?", choices)
    ...
    def _interact(self, text, choices):
        while True:
            ...
            actions = [action for key, _, action in choices if key == inp]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "base", nargs=1, type=argparse.FileType("r"), help="base for 3-way diff"
    )
    parser.add_argument(
        "first", nargs=1, type=argparse.FileType("r"), help="First file to compare"
    )
    parser.add_argument(
        "second", nargs=1, type=argparse.FileType("r"), help="Second file to compare"
    )
    parser.add_argument(
        "-o",
        nargs=1,
        type=argparse.FileType("w"),
        help="Output of merge",
        dest="output",
    )
    parser.add_argument(
        "-i", action="store_true", help="Interactive resolution", dest="interactive"
    )
    parser.add_argument(
        "--install",
        action="store_true",
        help="Install into Git config (repo-local)",
        dest="install",
    )
    ...
    if "--install" in sys.argv:
        ...
        subprocess.run(
            ["git", "config", "--local", "mergetool.jsondiff.trustExitCode", "true"]
        )
        exit(0)
