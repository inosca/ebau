...
class DMSHandler:
    ...
    def generate_pdf(self, instance_id, request, form_slug=None, document_id=None):
        ...
        main_doc = Document.objects.get(
            **{"case__meta__camac-instance-id": instance_id}
        )

        municipality = "-"
        if dms_settings.get("MUNICIPALITY_SLUG"):
            try:
                municipality_id = main_doc.answers.get(
        ...
        if dms_settings.get("ADD_HEADER_DATA"):
            header_data = {
                "addressHeader": CalumaApi().get_address(main_doc),
                ...
                "inputDateHeader": get_header_input_date(main_doc.case),
                "descriptionHeader": get_header_description(
                    main_doc, "beschreibung-bauvorhaben"
                ),
                "modificationHeader": get_header_description(
                    main_doc, "beschreibung-projektaenderung"
                ),
            }
