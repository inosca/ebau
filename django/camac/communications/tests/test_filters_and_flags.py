...
@pytest.mark.parametrize("role__name", ["Municipality"])
@pytest.mark.parametrize(
    "filter, expect_set", [(True, "draft"), (False, "sent"), (None, "all")]
)
def test_draft_filter(
    db, admin_client, communications_message_factory, be_instance, filter, expect_set
):
    my_entity = admin_client.user.groups.first().service_id
    sent_messages = communications_message_factory.create_batch(
        2,
        topic__involved_entities=[my_entity],
        topic__instance=be_instance,
        sent_at="2022-12-12T12:12:12Z",
    )
    draft_messages = communications_message_factory.create_batch(
        2,
        topic__involved_entities=[my_entity],
        topic__instance=be_instance,
        sent_at=None,
    )
    all_messages = sent_messages + draft_messages

    expected_sets = {
        "sent": [str(msg.pk) for msg in sent_messages],
        "draft": [str(msg.pk) for msg in draft_messages],
        "all": [str(msg.pk) for msg in all_messages],
    }

    expected_ids = set(expected_sets[expect_set])

    filter_params = {"is_draft": filter} if filter is not None else {}
    ...
    if expect_set == "all":
        ...
        # listed messages
        for msg_obj in resp.json()["data"]:
            is_sent = msg_obj["attributes"]["sent-at"]
            if is_sent:
                assert msg_obj["id"] in expected_sets["sent"]
            else:
                assert msg_obj["id"] in expected_sets["draft"]
