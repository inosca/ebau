...
def test_create_afu_work_item_so(
    ...
    service.slug = "afu"
    service.save()
    afu_child_1 = service_factory(service_parent=service)
    afu_child_2 = service_factory(service_parent=service)
    ...
    # Fails check that the service has a slug
    _grant(
        service=not_afu,
    )
    assert not afu_workitem().exists()
    ...
    wi = afu_workitem()
    assert wi.count() == 1
    assert wi.first().addressed_groups == [
        str(service.pk),
    ]
    assert wi.first().status == WorkItem.STATUS_READY
    ...
    send_event(
        ...
        sender="post_complete_work_item",
        work_item=caluma_work_item_factory(
            task_id="complete-instance", case=so_instance.case, child_case=None
        ),
        user=caluma_admin_user,
        context={},
    )
    assert afu_workitem().count() == 0
    assert afu_workitem(status=WorkItem.STATUS_COMPLETED).count() == 1

    # Other workitem form different case was not closed
    other_case_wi.refresh_from_db()
    assert other_case_wi.status == WorkItem.STATUS_READY
