...
import caseTableConfig from "ebau/config/case-table";
...
export default class CaseTableComponent extends Component {
  ...
  get isService() {
    return this.session.rolePermission === "service";
  }
  ...
  get tableColumns() {
    ...
    const role = this.session.rolePermission;
  ...
  redirectToCase(caseRecord) {
    this.router.transitionTo("cases.detail", caseRecord.instanceId);
  }
}
