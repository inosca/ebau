  ...
  municipalities: computed("data.lastSuccessful.value", function() {
    return this.getWithDefault("data.lastSuccessful.value", [])
      .map(edge => edge.node.document.answers.edges
        ...
        .map(edge => edge.node.formValue.answers.edges
          .map(edge => {
    ...
    return this.getWithDefault("data.lastSuccessful.value", [])
      .map(edge => edge.node.document.answers.edges
        ...
        .map(edge => edge.node.formValue.answers.edges
          .map(edge => {
            const answer = edge.node.formValue.answers.edges.find(edge =>
              edge.node.question.slug === SLUG_DESCRIPTION
            );
            return answer && answer.node.stringValue;
          })
          .filter(answer => answer !== undefined)[0]
  ...
  addresses: computed("data.lastSuccessful.value", function() {
    return this.getWithDefault("data.lastSuccessful.value", [])
      .map(edge => edge.node.document.answers.edges
        ...
        .map(edge => edge.node.formValue.answers.edges
          .map(edge => {
            const answer_street = edge.node.formValue.answers.edges.find(edge =>
              edge.node.question.slug === SLUG_ADDRESS_STREET
            );
            const answer_nr = edge.node.formValue.answers.edges.find(edge =>
              edge.node.question.slug === SLUG_ADDRESS_NR
            );
            const answer_locality = edge.node.formValue.answers.edges.find(edge =>
              edge.node.question.slug === SLUG_ADDRESS_LOCALITY
            );

            const value_street = answer_street && answer_street.node.stringValue;
            const value_nr = answer_nr && answer_nr.node.stringValue;
            const value_locality = answer_locality && answer_locality.node.stringValue;
