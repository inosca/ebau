<#import "template.ftl" as layout>
    <@layout.registrationLayout displayMessage=!messagesPerField.existsError('password') hideOtherWays=true displayInfo=(realm.password && realm.registrationAllowed && !registrationDisabled??); section>
        <#if section=="header">
            ${msg("loginPasswordTitle")}
            <#elseif section=="form">
                <#if realm.password && realm.loginWithEmailAllowed>
                    <form class="sign-in-form" id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                        <div class="passwordField">
                            <input type="password" name="password" id="password" tabindex="2" autocomplete="off" placeholder="Passwort / Mot de passe" />
                        </div>
